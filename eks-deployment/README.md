...
1. **Use Spot Instances for worker nodes:**
   ```powershell
   eksctl create nodegroup --cluster microservices-eks --spot --instance-types t3.medium,t3.small
   ```
...
3. **Use smaller instance types:**
   - Switch to t3.small for development

4. **Stop cluster when not in use:**
   - Delete worker nodes (keep cluster)
